...
relaxed_heuristic("taxe_grille_2x3.txt")
relaxed_heuristic("dummy_graph.txt")
mini_main("taxe_grille_2x3.txt")
"""

function evaluate_solution(inputFile::String, T::Matrix{Float64}, silent::Bool=true)::Any
    println("Evaluating heuristic solution of ", inputFile)
    ...
    @objective(model, Max, sum(T[i, j] * n_k[k] * y[i, j, k] for (i, j, _) in eachrow(A_1) for k in 1:numberOfCommodities))
...
function relaxed_heuristic(inputFile::String, silent::Bool=false)
    """
    """
    ...
    if feasibleSolutionFound
        return isOptimal, solveTime, value, JuMP.value.(T)
    else
        println("Problem is not feasible !!!")
        return
    end
...
function mini_main(inputFile::String)
    _, _, _, T = relaxed_heuristic(inputFile)
    return evaluate_solution(inputFile, T)
end